...
domain/   ← Pure Kotlin. No Android/Firebase imports.
  ...
  service/    ← Domain service interfaces: LogService, NetworkService,
                EmbeddingService (MediaPipe-backed), EmbeddingIndex

data/     ← Firebase DTOs, Room entities, repository implementations
  local/        ← Room database (v5), DAOs, entities, mappers
    entity/       ← QuizEntity, QuizFtsEntity (FTS4), QuizEmbeddingProjection,
                    PendingSyncEntity (retry queue with retryCount/maxRetries)
    dao/          ← QuizDao with FTS MATCH and embedding queries
  ...
  worker/       ← WorkManager: BackendMaintenanceWorker, BackgroundSyncWorker,
                  EmbeddingIndexWorker (TFLite embedding generation)
  ml/           ← ModelManager (MediaPipe model lifecycle), TFLiteEmbeddingService
  search/       ← EmbeddingCache (in-memory vector index)
...
### Quiz Lifecycle (3 States)

1. **Draft**: `isDraft=true`, `isPublic=false` (forced) — work-in-progress, not visible to others
2. **Published (Private)**: `isDraft=false`, `isPublic=false` — finalized but only accessible via share code
3. **Public**: `isDraft=false`, `isPublic=true` — discoverable in search

Publishing a draft sets `isDraft=false` but does **not** force `isPublic=true`; visibility is a separate, explicit user toggle.

### Semantic Search Architecture (TFLite + FTS)

The app implements hybrid search combining full-text search (FTS4) and vector similarity:

- **FTS4**: `QuizFtsEntity` virtual table synced with `QuizEntity` for fast `MATCH` queries on title/description/tags
- **Embeddings**: 100-dim float vectors generated by on-device MediaPipe Universal Sentence Encoder (`universal_sentence_encoder.tflite`)
- **Indexing**: `EmbeddingIndexWorker` computes embeddings in batches, stores as ByteArray in `QuizEntity.embedding`
- **Search**: Combines FTS results with cosine similarity for relevance ranking
...
- Use Coil's `AsyncImage` for all network image loading
...
**Important**: When adding/modifying console commands in `domain/console/commands/`, the MCP server metadata in `mcp_server/command_registry.py` MUST be updated to match. Add a corresponding `CommandInfo` entry with matching name, aliases, usage, flags, examples, role, and category.
...
| Local DB | Room 2.8.3 (FTS4, TypeConverters) |
| Backend | Firebase (Auth, Firestore, Functions) |
| DI | Manual DI (no Hilt/Dagger) |
| Async | Kotlin Coroutines + Flow |
| Images | Coil |
| Fonts | Google Fonts (Playfair Display + Inter) |
| ML | MediaPipe (text embeddings) |
| Docs | Dokka 2.0.0 |
...
| `data/local/entity/QuizFtsEntity.kt` | FTS4 virtual table |
| `data/worker/EmbeddingIndexWorker.kt` | TFLite embedding background worker |
| `data/ml/TFLiteEmbeddingService.kt` | On-device embedding inference |
| `domain/service/EmbeddingService.kt` | Embedding service interface |
| `domain/service/EmbeddingIndex.kt` | Vector index interface |
| `domain/util/VectorSimilarity.kt` | Cosine similarity calculation |
| `domain/console/Command.kt` | Console command interface |
| `mcp_server/server.py` | MCP server for AI agents |
| `mcp_server/command_registry.py` | MCP command metadata (keep in sync!) |
...
- **MCP sync required** — update `command_registry.py` when modifying console commands
